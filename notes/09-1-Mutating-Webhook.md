...
`--programmatic-validation` 플래그가 Validating Webhook을 만든다.
...
| 파일                                            | 역할                                        |
| ----------------------------------------------- | ------------------------------------------- |
| `internal/webhook/v1/simpleapp_webhook.go`      | Webhook 로직 (Default 함수, TODO 채워야 함) |
| `internal/webhook/v1/simpleapp_webhook_test.go` | 테스트 파일                                 |
| `internal/webhook/v1/webhook_suite_test.go`     | 테스트 스위트                               |

기존 파일에도 변경이 생긴다:

| 파일                                | 변경 내용                   |
| ----------------------------------- | --------------------------- |
| `cmd/main.go`                       | Webhook 서버 등록 코드 추가 |
| `config/default/kustomization.yaml` | Webhook 관련 주석 해제 안내 |
| `config/webhook/`                   | Webhook 서버 설정 YAML 생성 |
| `config/certmanager/`               | TLS 인증서 설정 생성        |
| `PROJECT`                           | webhook: true 추가          |

### scaffold로 생성된 Default 함수 (아직 빈 상태)
